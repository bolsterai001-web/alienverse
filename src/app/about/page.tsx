    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-16 space-y-20">
      ...
      <section className="text-center space-y-4 max-w-3xl mx-auto">
        ...
        <h1 className="text-4xl sm:text-5xl font-black text-white tracking-tight">
          About the Universe of <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-violet-400">Alien Verse</span>
        </h1>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="space-y-4 text-slate-300 text-sm sm:text-base leading-relaxed">
            ...
            <p>
              When the deep salvage cutter <em className="text-cyan-300 font-semibold">Omen Horizon</em> drifts into the uncharted Kepler-452 Trench, its sensors pick up a phenomenon previously thought impossible: an unbroken carrier signal modulating along the hydrogen line.
            </p>
            <p>
      ...
      <section className="space-y-8">
        <div className="text-center max-w-2xl mx-auto space-y-2">
          ...
          <h2 className="text-3xl font-bold text-white">Cast of the Expedition</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {characters.map((char, i) => (
            <div
              key={i}
              className="p-6 sm:p-7 rounded-2xl bg-void-900/70 border border-slate-800 hover:border-cyan-500/40 transition-all duration-300 space-y-3"
            >
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-lg font-bold text-white">{char.name}</h3>
                  <p className="text-xs font-mono text-cyan-400">{char.role}</p>
                </div>
              ...
              <div className="pt-2 text-xs font-mono text-slate-400 border-t border-slate-800/80">
                <span className="text-cyan-400 font-bold">KEY ATTRIBUTE:</span> {char.specialty}
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="space-y-8">
        <div className="text-center max-w-2xl mx-auto space-y-2">
          ...
          <h2 className="text-3xl font-bold text-white">Core Themes & Science</h2>
          <p className="text-sm text-slate-400">
      ...
      <section className="p-8 sm:p-12 rounded-3xl bg-void-900/80 border border-cyan-500/30">
        <div className="max-w-3xl mx-auto space-y-6">
          <div className="text-center space-y-2">
            <h3 className="text-2xl font-bold text-white">Publication Technical Metadata</h3>
            <p className="text-xs font-mono text-slate-400">OFFICIAL ARCHIVE CATALOG: CELESTIAL PRESS</p>
          </div>

          <div className="divide-y divide-slate-800 text-sm">
            <div className="py-3 flex justify-between">
              <span className="text-slate-400">Full Title</span>
              <span className="font-semibold text-white">{BOOK_INFO.title}: {BOOK_INFO.subtitle}</span>
            </div>
            <div className="py-3 flex justify-between">
              <span className="text-slate-400">Author</span>
              <span className="font-semibold text-white">{BOOK_INFO.author}</span>
            </div>
            <div className="py-3 flex justify-between">
              <span className="text-slate-400">ISBN-13</span>
              <span className="font-mono text-cyan-300">{BOOK_INFO.specs.isbn}</span>
            </div>
            <div className="py-3 flex justify-between">
              <span className="text-slate-400">Total Extent</span>
              <span className="text-white">{BOOK_INFO.specs.pages} Pages (Trade & Hardcover)</span>
            </div>
            <div className="py-3 flex justify-between">
              <span className="text-slate-400">Publication Date</span>
              <span className="text-white">{BOOK_INFO.specs.published}</span>
            </div>
            <div className="py-3 flex justify-between">
              <span className="text-slate-400">Genres</span>
              <span className="text-white">{BOOK_INFO.specs.genres.join(", ")}</span>
            </div>
            <div className="py-3 flex justify-between">
              <span className="text-slate-400">Publisher</span>
              <span className="text-white">{BOOK_INFO.specs.publisher}</span>
            </div>
          </div>
        </div>
      </section>
